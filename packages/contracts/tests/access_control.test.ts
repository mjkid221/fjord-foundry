...
describe("Fjord LBP - Access Controls", () => {
  ...
  it("Should revoke the pending owner if the current owner revokes the nomination", async () => {
    ...
    expect(ownerConfig.owner.toBase58()).to.be.eq(creator.publicKey.toBase58());
    ...
    const newOwnerConfig = await program.account.ownerConfig.fetch(configPda);
    expect(newOwnerConfig.pendingOwner?.toBase58()).to.be.eq(
      testUserA.publicKey.toBase58()
    );

    // TODO: This should test a revoke method
  });

  it("Should not allow a non-pending owner to accept the new owner position", async () => {
    const [configPda] = findProgramAddressSync(
      [Buffer.from("owner_config")],
      program.programId
    );
